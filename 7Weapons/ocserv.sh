...
OUT=$?
if [ $OUT -eq 1 ];then
  print_info "dpkg Status normal"
else
  print_info "dpkg Status lock"
...
#install base-tools 
    ...
    apt-get update  -qq
    check_install "curl vim sudo gawk sed insserv nano openssl" "curl vim sudo gawk sed insserv nano openssl"
...
# Set Encryption algorithms	
   echo  -e "Choose a Encryption algorithms\n1: RSA_3DES_168_SHA1\n2: RSA_AES_256_SHA256\n3: ECDHE_RSA_AES256_GCM_SHA384\n"
    read text
	
	if [ $text -eq 1 ];then
	RSA_Encryption_Method=NONE:+VERS-TLS-ALL:+MAC-ALL:+RSA:+3DES-CBC:+SIGN-ALL:+COMP-NULL
    print_info "RSA_3DES_168_SHA1"
    fi
	
	if [ $text -eq 2 ];then
	RSA_Encryption_Method=NONE:+VERS-TLS-ALL:+MAC-ALL:+RSA:+AES-256-CBC:+SIGN-ALL:+COMP-NULL
    print_info "RSA_AES_256_SHA256"
    fi
	
	if [ $text -eq 3 ];then
	RSA_Encryption_Method=$"NORMAL:+VERS-TLS-ALL:+MAC-ALL:+RSA:+AES-256-CBC:+SIGN-ALL:+COMP-NULL"
    print_info "ECDHE_RSA_AES256_GCM_SHA384"
    print_info "$RSA_Encryption_Method"
...
#tcp-port only or not ......tcp........udp
    fast_Default_Ask "Only use tcp-port or not?(y/n)" "n" "only_tcp_port"
#which port to use for data transmission ..udp.. ........udp..
...
#gnutls-bin.certtool..wheezy/trusty/utopic...OU.........
    [ "$oc_D_V" = "wheezy" ] || {
...
#generating a local server key-certificate pair .....................
    openssl genrsa -out server-key.pem $RSA_Key_Size
...
#user key
    openssl genrsa -out user-${name_user_ca}/user-${name_user_ca}-key.pem $RSA_Key_Size
#user cert
...
#set port
    ...
    sed -i "s|\NORMAL:%SERVER_PRECEDENCE:%COMPAT:-VERS-SSL3.0|$RSA_Encryption_Method|" /etc/ocserv/ocserv.conf
    sed -i "s|\udp-port|#udp-Port-snowleopard|" /etc/ocserv/ocserv.conf
